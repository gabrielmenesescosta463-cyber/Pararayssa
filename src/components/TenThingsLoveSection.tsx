...
import { Heart, Sparkles, Edit3, Check, X } from 'lucide-react';
import confetti from 'canvas-confetti';
import { HeartBalloonItem, TenThingsReasonItem } from '../types';

interface TenThingsLoveSectionProps {
  partnerName?: string;
}
...
export const TenThingsLoveSection: React.FC<TenThingsLoveSectionProps> = ({
  ...
  // Active balloon pop-up modal state
  ...
  return (
    <div className="w-full pt-4 pb-10 space-y-6">
      {/* Romantic Hero Card with Scroll Reveal & Title Fade Animation */}
      <motion.div
        initial={{ opacity: 0, y: 35, filter: 'blur(8px)' }}
        whileInView={{ opacity: 1, y: 0, filter: 'blur(0px)' }}
        viewport={{ once: false, amount: 0.2 }}
        transition={{ duration: 0.85, ease: [0.16, 1, 0.3, 1] }}
        className="text-center bg-white/90 backdrop-blur-sm rounded-3xl p-6 sm:p-8 border border-pink-200/80 shadow-sm relative overflow-hidden"
      >
        <motion.div
          ...
          className="w-12 h-12 mx-auto rounded-full bg-rose-100 text-rose-500 flex items-center justify-center mb-2 shadow-inner"
        >
        ...
        {/* Dedication Subtitle with Fade Animation */}
        <motion.p
          initial={{ opacity: 0, y: 10 }}
          whileInView={{ opacity: 1, y: 0 }}
          viewport={{ once: false }}
          transition={{ delay: 0.4, duration: 0.7, ease: 'easeOut' }}
          className="text-xs sm:text-sm text-pink-700/80 max-w-md mx-auto mt-2 leading-relaxed"
        >
      ...
      {/* The 10 Reasons with 5 Small Heart Balloons Spread Around */}
      <motion.div
      ...
      >
        ...
        {/* Grid of the 10 Reasons with Small Heart Balloons Spread Around */}
